        ...
        if len(repo_id) > 0:
            ...
            if not os.path.isdir(target_dir):
                try:
                    ...
                    print(f"Downloading model from ModelScope repo '{repo_id}' to '{target_dir}' ...", file=sys.stderr)
                    snapshot_download(repo_id=repo_id, local_dir=target_dir)
                except Exception as e:
                    print(f"Warning: ModelScope download failed: {e}. Falling back to 'data'.", file=sys.stderr)
                    return "models"
            return target_dir
        return "models"
